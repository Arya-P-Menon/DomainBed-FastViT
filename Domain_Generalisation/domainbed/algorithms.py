...
def focal_loss(inputs, targets, alpha=0.25, gamma=2.0, reduction='mean'):
    ...
    Args:
        inputs (torch.Tensor): Raw logits from the model (N, C).
    ...
    # Create one-hot targets for alpha weighting
    ...
    # Get alpha for the true class
    alpha_for_true_class = alpha_t.gather(1, targets.unsqueeze(1)).squeeze(1)

    loss = alpha_for_true_class * focal_term * ce_loss
...
class FastViT(Algorithm):

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(FastViT, self).__init__(input_shape, num_classes, num_domains,
                                               hparams)
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            self.optimizer, T_max=self.n_steps
        )
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        cross_entropy_component = F.cross_entropy(output, all_y)
        focal_loss_component = focal_loss(
            output, all_y,
            alpha=0.25,
            gamma=2.0,
            reduction='mean'
        )
        # Combine CE and Focal Loss for the base loss
        base_loss = (1 - 0.2) * cross_entropy_component + 0.2 * focal_loss_component
        #--------------------------------------------------------------------------
        ...
        loss.backward()
        self.optimizer.step()
        self.scheduler.step()  ###Addition


        return {'loss': loss.item(), 'ce_loss': cross_entropy_component.item()}
...
def return_backbone_network(network_name, num_classes, hparams):
    if (network_name == "DeitSmall"):
        ...
        network.head = nn.Linear(384, num_classes)
        return network
    elif (network_name == "fastViT"):
        return HuggingFaceFastViTBackbone(num_classes=num_classes)
