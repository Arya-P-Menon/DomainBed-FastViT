...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, norm_layer=None, drop=0.):
        super().__init__()
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.GELU(),
            norm2d(hidden_features, eps=1e-5),
        )
        self.proj = nn.Conv2d(hidden_features, hidden_features, 3, 1, 1, groups=hidden_features)
        self.proj_act = nn.GELU()
        self.proj_bn = norm2d(hidden_features, eps=1e-5)
        self.conv2 = nn.Sequential(
            nn.Conv2d(hidden_features, out_features, 1, 1, 0, bias=True),
            norm2d(out_features, eps=1e-5),
        )
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, norm_layer=norm_layer)
...
class CMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[46, 92, 184, 368], stem_channel=16,
        ...
        super().__init__()
        norm_type = 'ln'  # or 'bn' — configurable
        Norm2d = nn.BatchNorm2d if norm_type == 'bn' else LayerNorm2d
        Norm1d = nn.LayerNorm  # always use LayerNorm for 1D
        ...
        self.stem_norm1 = self.Norm2d(stem_channel, eps=1e-5)
        ...
        self.stem_norm2 = self.Norm2d(stem_channel, eps=1e-5)
        ...
        self.stem_norm3 = self.Norm2d(stem_channel, eps=1e-5)
        ...
        self.blocks_a = nn.ModuleList([
            Block(
                ...
                norm_layer=self.Norm1d, qk_ratio=qk_ratio, sr_ratio=sr_ratios[0])
        ...
        self.blocks_b = nn.ModuleList([
            Block(
                ...
                qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],
                norm_layer=self.Norm1d, qk_ratio=qk_ratio, sr_ratio=sr_ratios[1])
        ...
        self.blocks_c = nn.ModuleList([
            Block(
                ...
                qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],
                norm_layer=self.Norm1d, qk_ratio=qk_ratio, sr_ratio=sr_ratios[2])
        ...
        self.blocks_d = nn.ModuleList([
            Block(
                ...
                qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i],
                norm_layer=self.Norm1d, qk_ratio=qk_ratio, sr_ratio=sr_ratios[3])
        ...
        # Classifier head
        # Per-stage classifier projection heads
        self._fc_list = nn.ModuleList([
            nn.Conv2d(ed, fc_dim, kernel_size=1) for ed in embed_dims
        ])
        self._bn_list = nn.ModuleList([
            norm_layer(fc_dim, eps=1e-5) for _ in embed_dims
        ])
    ...
    def forward_all_features(self, x):
        ...
        feat_a = self._avg_pooling(self._swish(self._bn_list[0](self._fc_list[0](x_)))).flatten(start_dim=1)
        features_list.append(feat_a)

        # Block B
        x, (H, W) = self.patch_embed_b(x_)
        for blk in self.blocks_b:
            x = blk(x, H, W, self.relative_pos_b)
        x_ = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        feat_b = self._avg_pooling(self._swish(self._bn_list[1](self._fc_list[1](x_)))).flatten(start_dim=1)
        features_list.append(feat_b)

        # Block C
        x, (H, W) = self.patch_embed_c(x_)
        for blk in self.blocks_c:
            x = blk(x, H, W, self.relative_pos_c)
        x_ = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        feat_c = self._avg_pooling(self._swish(self._bn_list[2](self._fc_list[2](x_)))).flatten(start_dim=1)
        features_list.append(feat_c)

        # Block D
        x, (H, W) = self.patch_embed_d(x_)
        for blk in self.blocks_d:
            x = blk(x, H, W, self.relative_pos_d)
        x_ = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        feat_d = self._avg_pooling(self._swish(self._bn_list[3](self._fc_list[3](x_)))).flatten(start_dim=1)
        features_list.append(feat_d)

        # Head projection
        outputs = [self.head(f) for f in features_list]
        return outputs

    def forward(self, x, return_feat=False):
        return self.forward_all_features(x)
...
def _create_cmt_model(pretrained=False, network='XS', distilled=False, **kwargs):
    default_cfg = _cfg()
    ...
    if pretrained and network == 'XS':
        print("Loading pretrained XS_LN")
        model.load_state_dict(torch.load("./domainbed/pretrained_models/cmt/cmt_xs.pth", map_location="cpu"),
                              strict=False)
    elif pretrained and network == 'T':
        model.load_state_dict(torch.load("./domainbed/pretrained_models/cmt/cmt_tiny.pth", map_location="cpu"),
                              strict=False)
    return model
...
def cmt_ti(pretrained=False, **kwargs):
    ...
    model = _create_cmt_model(pretrained=pretrained, network='T', **model_kwargs)
    return model
...
def cmt_xs(pretrained=False, **kwargs):
    ...
    model = _create_cmt_model(pretrained=pretrained, network='XS', **model_kwargs)
    return model
