...
class CMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[46,92,184,368], stem_channel=16, fc_dim=1280,
        ...
        self._bn_list = nn.ModuleList([
            nn.BatchNorm2d(fc_dim, eps=1e-5) for _ in embed_dims
        ])
...
def _create_cmt_model(pretrained=False, network='XS', distilled=False, **kwargs):
    ...
    if pretrained and network=='XS':
        model.load_state_dict(torch.load("./domainbed/pretrained_models/cmt/cmt_xs.pth", map_location="cpu"), strict=False)
    elif pretrained and network=='T':
        model.load_state_dict(torch.load("./domainbed/pretrained_models/cmt/cmt_tiny.pth", map_location="cpu"), strict=False)
    return model
