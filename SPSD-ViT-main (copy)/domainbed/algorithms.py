...
from domainbed.lib.cmt import cmt_ti, cmt_xs
...
def return_backbone_network(network_name,num_classes,hparams):
    if(network_name=="DeitSmall"):
        ...
        network.head = nn.Linear(384, num_classes)
        return network
    elif(network_name=="CMTXSmall"):
        network = cmt_xs(pretrained=True)
        in_features = network.head.in_features
        network.head = nn.Linear(in_features, num_classes)
        return network
    elif(network_name=="CMTTiny"):
        network = cmt_ti(pretrained=True)
        in_features = network.head.in_features
        network.head = nn.Linear(in_features, num_classes)
        return network
